...
export default function About({ params: { locale } }: { params: { locale: string } }) {
  ...
  const structure = [
    ...
    },
    {
      ...
      items: about.additional.items.map((experience) => experience.name),
    },
    {
      ...
      <Flex fillWidth mobileDirection="column" justifyContent="center">
        ...
        <Flex className={styles.blockAlign} fillWidth flex={9} maxWidth={40} direction="column">
          <Flex id={about.intro.title} fillWidth minHeight="160" direction="column" justifyContent="center" marginBottom="32">
            ...
            <Text className={styles.textAlign} variant="display-default-xs" paddingTop="12" onBackground="neutral-weak">
              {person.role}
            </Text>
            {social.length > 0 && (
          ...
          {about.additional.display && (
            <>
              <Heading as="h2" id={about.additional.title} variant="display-strong-s" marginBottom="m">
                {about.additional.title}
              </Heading>
              <Flex direction="column" fillWidth gap="l" marginBottom="40">
                {about.additional.items.map((item, index) => (
                  <Flex key={`${item.name}-${item.timeframe}-${index}`} fillWidth direction="column">
                    <Flex fillWidth justifyContent="space-between" alignItems="flex-end" marginBottom="4">
                    ...
                    <Flex as="ul" direction="column" gap="16">
                      {item.achievements.map((achievement: string, index: any) => (
                        <Text as="li" variant="body-default-m" key={`${item.name}-${index}`}>
                          {achievement}
                        </Text>
                      ))}
                    </Flex>
